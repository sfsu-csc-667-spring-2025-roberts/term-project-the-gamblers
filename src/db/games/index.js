...
const getGames = async (user_id) => {
  const games = await db.query("SELECT * FROM games WHERE owner_id != $1", [
    user_id,
  ]);
...
const joinGame = async (game_id, user_id, password) => {
  const CONDITIONALLY_JOIN_SQL = `
    ...
    `;
  const result = await db.query(CONDITIONALLY_JOIN_SQL, [
    game_id,
    user_id,
    password,
  ]);
...
const getGameById = async (game_id) => {
  const result = await db.query("SELECT * FROM games WHERE game_id = $1", [
    game_id,
  ]);
...
const removeGame = async (game_id, user_id) => {
  ...
  const result = await db.query(
    "DELETE FROM games WHERE game_id = $1 AND owner_id = $2",
    [game_id, user_id],
  );
